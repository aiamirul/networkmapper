...
import { BrainCircuitIcon, SettingsIcon, MenuIcon } from './icons/Icons';
...
export const Header: React.FC<HeaderProps> = ({ macFormat, setMacFormat, onAnalyzeClick, onOpenSettings, onToggleMobileMenu }) => {
  return (
    <header className="bg-slate-900/80 backdrop-blur-sm border-b border-slate-700/50 h-16 flex items-center justify-between px-4 md:px-6 sticky top-0 z-10 shrink-0">
      <div className="flex items-center gap-2">
        <button 
            onClick={onToggleMobileMenu}
            className="md:hidden p-2 -ml-2 rounded-full hover:bg-slate-700 transition-colors"
            title="Open Menu"
        >
